...
export default function Editor() {
    ...
    const updateSelectionUI = (item: paper.Item | null) => {
        ...
        if (item.className === 'PointText') {
            ...
            const text = item.children.find(c => c.className === 'PointText') as paper.PointText;
    ...
    const finishInlineEdit = () => {
        ...
        if (item) {
            if (item.className === 'PointText') {
                ...
                const t = item.children.find(c => c.className === 'PointText') as paper.PointText;
    ...
    useEffect(() => {
        ...
        toolInstance.onMouseDown = (event: paper.ToolEvent) => {
            ...
            if (currentTool === 'select') {
                ...
                if (hit && hit.item) {
                    ...
                    const isText = target.className === 'PointText' || (target.className === 'Group' && target.children.some(c => c.className === 'PointText'));
                    if (isDoubleClick && isText) {
                        let textItem = target.className === 'PointText' ? target as paper.PointText : target.children.find(c => c.className === 'PointText') as paper.PointText;
            ...
            if (dragState.current.mode === 'moving' && dragState.current.item) {
                ...
                if (path.data.type === 'line') {
                    ...
                    if (dimensionText.current) {
                        ...
                        dimensionText.current.point = path.getPointAt(path.length / 2).add([0, -10]);
                    }
                    ...
                    if (dimensionText.current) {
                        ...
                        dimensionText.current.point = newRect.bounds.center;
                    }
                    ...
                    if (dimensionText.current) {
                        dimensionText.current.content = `R: ${(radius / SCALE_MM_TO_PX).toFixed(1)}mm`;
                        dimensionText.current.point = newPoly.position;
                    }
                }
            }
        };
        ...
        const handleCadAction = (e: any) => {
            ...
            const selected = selectedIds
                .map(id => project.getItem({ id: parseInt(id) }))
                ...
                selected.forEach(item => {
                    ...
                    if (props.content !== undefined) {
                        ...
                        if (item.className === 'PointText') {
                            ...
                            textItem = item.children.find(c => c.className === 'PointText') as paper.PointText;
                        }
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'z') {
                ...
                if (item) {
                    const isText = item.className === 'PointText' || (item.className === 'Group' && item.children.some(c => c.className === 'PointText'));
                    ...
                    if (isText && (e.key.length === 1 || e.key === 'Enter') && !e.ctrlKey && !e.metaKey && !e.altKey) {
                        let textItem = item.className === 'PointText' ? item as paper.PointText : item.children.find(c => c.className === 'PointText') as paper.PointText;
                        const viewPos = paper.view.projectToView(textItem.point);
                        setInlineEdit({
                            id: item.id.toString(),
    ...
    // Undo/Redo Effect
    const version = useCadStore(s => s.version);
    useEffect(() => {
        if (!paperScope.current) return;
        const project = paperScope.current.project;
    ...
    const drawGrid = () => {
        ...
        // Re-init grid layer
        const oldGrid = paper.project.layers.find(l => l.name === 'grid');
        ...
        let main = paper.project.layers.find(l => l.name === 'main');
        if (!main) main = new paper.Layer({ name: 'main' });
        main.activate();
    };

    const updateProjectTree = () => {
        const items = paper.project.layers.find(l => l.name === 'main')?.children || [];
        setProjectTree(items.map(i => ({
            id: i.id.toString(),
